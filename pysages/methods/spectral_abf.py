...
def _spectral_abf(method, snapshot, helpers):
    ...
    def update(state, data):
        ...
        #
        return SpectralABFState(
            xi, bias, hist, Fsum, force, Wp, state.Wp, fun, state.nstep + 1
        )
...
def analyze(result: Result[SpectralABF]):
    ...
    dict:
        ...
        free_energy: Callable[[JaxArray], JaxArray]
    ...
    if len(states) == 1:
        ...
        free_energy = build_fes_fn(states[0].fun)

        return dict(hist=hist, mean_force=mean_force, mesh=mesh, free_energy=free_energy)
    ...
    for s in states:
        hists.append(s.hist)
        mean_forces.append(average_forces(hist, s.Fsum))
        free_energies.append(build_fes_fn(s.fun))

    return dict(hist=hists, mean_force=mean_forces, mesh=mesh, free_energy=free_energies)
